...
The service has its own **`deployment/`** folder: manifest `deployment/email-service.yaml` and scripts **`deployment/scripts/deploy.sh`**, **`deployment/scripts/build-and-push.sh`**. Shared cluster config (namespace, ConfigMap, Secrets, Traefik) lives in **`shared-services-configuration/deployment/`**.
